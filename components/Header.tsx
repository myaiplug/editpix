...
const Header: React.FC<HeaderProps> = ({ onSettingsClick, isAdminMode }) => {
  return (
    <header className="w-full py-4 px-8 border-b border-gray-700 bg-gray-800/30 backdrop-blur-sm sticky top-0 z-50">
      <div className="flex items-center justify-between max-w-[1600px] mx-auto">
        <div className="flex items-center gap-3">
          <SparkleIcon className="w-6 h-6 text-[#7C4DFF]" />
          <h1 className="text-xl font-bold tracking-tight text-gray-100">
            EditPix 
          </h1>
          {isAdminMode && (
            <span className="bg-gradient-to-r from-[#7C4DFF] to-[#00C2FF] text-white text-xs font-bold px-2 py-1 rounded-full">
              ADMIN
            </span>
          )}
        </div>
