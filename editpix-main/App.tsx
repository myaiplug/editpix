...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!prompt.trim()) {
        toast.error('Please enter a description for your edit.');
        soundManager.playError();
        return;
    }

    if (!editHotspot) {
        toast.error('Please click on the image to select an area to edit.');
        soundManager.playError();
        return;
    }
    ...
    try {
        const editedImageUrl = await generateEditedImage(currentImage, prompt, editHotspot, selectedModel);
        ...
        toast.error(`Failed to generate the image. ${errorMessage}`);
        soundManager.playError();
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  }, [currentImage, prompt, editHotspot, addImageToHistory, selectedModel]);
  
  const handleApplyFilter = useCallback(async (filterPrompt: string) => {
    if (!currentImage) {
      toast.error('No image loaded to apply a filter to.');
      soundManager.playError();
      return;
    }
    ...
    try {
        const filteredImageUrl = await generateFilteredImage(currentImage, filterPrompt, selectedModel);
        const newImageFile = dataURLtoFile(filteredImageUrl, `filtered-${Date.now()}.png`);
        addImageToHistory(newImageFile);
        soundManager.playSuccess();
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        toast.error(`Failed to apply the filter. ${errorMessage}`);
        soundManager.playError();
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  }, [currentImage, addImageToHistory, selectedModel]);
  
  const handleApplyAdjustment = useCallback(async (adjustmentPrompt: string) => {
    if (!currentImage) {
      toast.error('No image loaded to apply an adjustment to.');
      soundManager.playError();
      return;
    }
    ...
    try {
        const adjustedImageUrl = await generateAdjustedImage(currentImage, adjustmentPrompt, selectedModel);
        const newImageFile = dataURLtoFile(adjustedImageUrl, `adjusted-${Date.now()}.png`);
        addImageToHistory(newImageFile);
        soundManager.playSuccess();
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        toast.error(`Failed to apply the adjustment. ${errorMessage}`);
        soundManager.playError();
        console.error(err);
    } finally {
        setIsLoading(false);
  ...
  const handleGenerateImage = useCallback(async (prompt: string, aspectRatio: string) => {
    ...
    try {
        const generatedImageUrl = await generateImageFromText(prompt, aspectRatio, selectedModel);
  ...
  }, [history, addImageToHistory, selectedModel]);
  ...
  const handleGenerateLogo = useCallback(async (prompt: string, style: string) => {
    ...
    try {
        const generatedImageUrl = await generateImageFromText(prompt, '1:1', selectedModel);
        ...
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        toast.error(`Failed to generate the logo. ${errorMessage}`);
        soundManager.playError();
        console.error(err);
    } finally {
        setIsLoading(false);
  ...
  const handleGenerateFavicon = useCallback(async (prompt: string) => {
    ...
    try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        toast.error(`Failed to generate the favicon. ${errorMessage}`);
        soundManager.playError();
        console.error(err);
    } finally {
        setIsLoading(false);
  ...
  const handleApplyCrop = useCallback(() => {
    ...
    if (!ctx) {
        toast.error('Could not process the crop.');
        return;
    }
